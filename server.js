...
app.get('/api/signup/confirm', async (req, res) => {
  try {
    ...
    // Check the REAL subscription state so the frontend can say something
    // accurate — "payment confirmed" is simply false during a free trial,
    // since Stripe hasn't actually charged anything yet.
    let isTrialing = false, trialEnd = null;
    if (session.subscription) {
      try {
        ...
        // Not fatal — worst case the frontend falls back to the generic message.
      }
    }
