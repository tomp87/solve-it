  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      if (classId && session.customer && session.subscription) {
        ...
        const sourcePage = (session.metadata && session.metadata.sourcePage) || null;
        const wasExcluded = session.metadata && session.metadata.notrack === '1';
        if (!wasExcluded) {
          try { await db.logAnalyticsEvent({ eventType: 'converted', page: sourcePage }); } catch (e) { console.error('Analytics log failed:', e); }
        }
      }
...
app.post('/api/signup/start', async (req, res) => {
  try {
    ...
    const { email, passcode, accountName, page, notrack } = req.body || {};
    ...
    const sourcePage = page ? String(page).slice(0, 60) : 'signup';
    const excludeFromAnalytics = notrack === true;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // sourcePage rides along in metadata so the webhook — which fires
      // later, sometimes minutes later — can still attribute the eventual
      // conversion to the same landing page that started it. notrack rides
      // along the same way, so a real test signup (which happens for real
      // money/data, unlike a page view) doesn't quietly count as a genuine
      // conversion once the webhook fires.
      metadata: { classId: String(created.id), planId: process.env.STRIPE_PRICE_ID, sourcePage, notrack: excludeFromAnalytics ? '1' : '' },
    ...
    if (!excludeFromAnalytics) {
      try { await db.logAnalyticsEvent({ eventType: 'checkout_started', page: sourcePage }); } catch (e) { console.error('Analytics log failed:', e); }
    }
